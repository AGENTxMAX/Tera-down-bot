...
def format_size(size):
    ...
    size = remove_ansi(size)  # Remove ANSI sequences
    if not size.isdigit():  # Handle non-numeric cases like "Unknown B/s"
        return "Unknown"
...
def format_progress_bar(filename, percentage, done, total_size, status, eta, speed, elapsed, user_mention, user_id):
    ...
    def format_time(seconds):
        ...
        seconds = remove_ansi(seconds)  # Remove ANSI if any
        if not str(seconds).isdigit() or int(seconds) <= 0:
            return "N/A"
        seconds = int(seconds)
        if seconds < 60:
